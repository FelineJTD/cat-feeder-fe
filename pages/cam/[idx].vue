...
<template>
  <div>
    ...
    <div class="h-screen w-full flex items-end justify-between py-16 mobile-landscape:py-8 px-8 mobile-landscape:px-8 md:px-16 lg:px-32 text-white">
      <!-- INFO -->
      <div class="flex h-1/2 items-end">
        <!-- INDICATOR -->
        <div class="flex flex-col gap-2 items-center mr-4 mobile-landscape:mr-4 md:mr-12">
          <div v-for="idx in indicatorArr" :key="idx" class="h-20 w-1" :class="(idx === parseInt(route.params.idx)) ? 'bg-primary' : 'bg-white opacity-50'" />
        </div>
        <div>
          <div class="flex gap-0 mobile-landscape:gap-0 md:gap-4 items-center -ml-2">
          ...
          <h1 class="text-primary mt-0 mobile-landscape:mt-0 md:mt-4 mobile-landscape:text-3xl">{{ camName }}</h1>
      ...
      <!-- KASIH MAKAN BUTTON -->
      <button class="duration-150 aspect-square rounded-full text-xl mobile-landscape:text-xl md:text-2xl lg:text-4xl h-24 mobile-landscape:h-24 md:h-36 lg:h-52 bg-primary font-header text-black shadow-[-8px_8px] hover:shadow-none hover:-translate-x-2 -translate-y-2 hover:translate-y-0" @click="feed">Kasih makan</button>
      <!-- LOGO -->
      <NuxtLink to="/" class="h-[60px] w-fit absolute top-16 mobile-landscape:top-8">
